...
model = model.cuda()
model.eval()
model.load_state_dict(torch.load("output/video_sr_2_x8/best.pth"))

demo_video = "E:/video_set/7.avi"  # 49 50
scale = 8
order = 0
temp = np.zeros((5, 28, 28, 3))
cap = cv2.VideoCapture(demo_video)  # 打开视频

fourcc = cv2.VideoWriter_fourcc(*'XVID')
out_frame = cv2.VideoWriter('5_frame.avi', fourcc, 20.0, (1280, 720), True)
...
while(True):
    ...
    if order == 0:
        frame_GT = [frame_original, frame_original, frame_original, frame_original, frame_original]
        frame_bicu = [bicubic, bicubic, bicubic, bicubic, bicubic]
    else:
        frame_GT = [frame_GT[1], frame_GT[2], frame_GT[3], frame_GT[4], frame_original]
        frame_bicu = [frame_bicu[1], frame_bicu[2], frame_bicu[3], frame_bicu[4], bicubic]
    ...
    if ret:
        ...
        if order == 0:
            order = order + 1
            print('Initial...')
            video_fifo = [frame, frame, frame, frame, frame]
            box,_ = mtcnn_detector.detect_face(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            box_fifo = [box, box, box, box, box]
        else:
            video_fifo = [video_fifo[1], video_fifo[2], video_fifo[3], video_fifo[4], frame]
            box, _ = mtcnn_detector.detect_face(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            box_fifo = [box_fifo[1], box_fifo[2], box_fifo[3], box_fifo[4], box]
            order = order + 1
            if (len(box_fifo[0]) == 0)|(len(box_fifo[1]) == 0)|(len(box_fifo[2]) == 0)|(len(box_fifo[3]) == 0)|(len(box_fifo[4]) == 0):
                print('The {} frame can not detect face'.format(order))
                frame_bicu[2] = cv2.cvtColor(frame_bicu[2], cv2.COLOR_BGR2RGB)
                cv2.imshow('bicubic', frame_bicu[2])
                cv2.imshow('video_sr', frame_bicu[2])
                cv2.waitKey(1)
            else:
                r = max(box_fifo[0][0][2] - box_fifo[0][0][0], box_fifo[0][0][3] - box_fifo[0][0][1],
                        ...
                        box_fifo[2][0][2] - box_fifo[2][0][0], box_fifo[2][0][3] - box_fifo[2][0][1],
                        box_fifo[3][0][2] - box_fifo[3][0][0], box_fifo[3][0][3] - box_fifo[3][0][1],
                        box_fifo[4][0][2] - box_fifo[4][0][0], box_fifo[4][0][3] - box_fifo[4][0][1])  # 取3帧里面最大的框
                ...
                input = np.concatenate((temp[0], temp[1], temp[2], temp[3], temp[4]), axis=2).transpose(2, 0, 1)
                ...
                frame_bicu[2] = cv2.cvtColor(frame_bicu[2], cv2.COLOR_BGR2RGB)
                cv2.imshow('bicubic', frame_bicu[2])
                center_x = (box_fifo[2][0][0] + box_fifo[2][0][2]) / 2
                center_y = (box_fifo[2][0][1] + box_fifo[2][0][3]) / 2
                left_x = center_x - r / 2
                left_y = center_y - r / 2
                right_x = center_x + r / 2
                right_y = center_y + r / 2
                ...
                frame_bicu[2][int(left_y)*scale:int(right_y)*scale, int(left_x)*scale:int(right_x)*scale, :] = out_cat
                cv2.imshow('video_sr', frame_bicu[2])
                # print(time.time() - t)
                cv2.waitKey(1)
            out_frame.write(frame_bicu[2])
